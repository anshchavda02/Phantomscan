...
_KNOWN_LIBRARIES: dict[str, re.Pattern[str]] = {
    ...
    "AngularJS": re.compile(r"angular\.js.*?([\d.]+)", re.I),
    "React": re.compile(r"react(?:\.production)?[.\-/]?v?([\d.]+)", re.I),
    "Vue.js": re.compile(r"vue[.\-/]?v?([\d.]+)", re.I),
    "Bootstrap": re.compile(r"bootstrap[.\-/]?v?([\d.]+)", re.I),
    "Lodash": re.compile(r"lodash[.\-/]?v?([\d.]+)", re.I),
    "Moment.js": re.compile(r"moment[.\-/]?v?([\d.]+)", re.I),
}
            ...
            if matches:
                ...
                if real_matches:
                    ...
                    findings.append({
                        ...
                        "title": f"Hardcoded {secret_type} Exposed in JavaScript",
                        "severity": "high" if "live" in secret_type.lower() or "AWS" in secret_type else "medium",
                        "confidence": "high" if secret_type in ("AWS Access Key ID", "Stripe Live Secret Key", "GitHub Personal Token") else "medium",
                        "category": "supply-chain",
                        "target": target,
                        "evidence": (
